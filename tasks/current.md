...
| T40 | **「今日のメニュー」セッション化**(基盤→ボイトレ第1段、ユーザー承認): ウォームアップ→データ駆動の重点2つ→しあげ、の10分ガイド。編成=core/menu/buildDailyMenu(ルールベース、将来AI Coach差替可) | Fable(設計)/Sonnet(実装)/**Codex(差分クロスレビュー予定)** | 🔄 実装中(仕様=TRAINING_MODEL「今日のメニュー」/UX 5e) |
