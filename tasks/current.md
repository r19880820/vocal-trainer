...
| T33 | 音域チェック実地検証の追修正: タイミング合図(👂/🎤)+自動リトライ+失敗使い回し禁止 → 「楽」に安定性σ≤50条件+上限打ち切り明示 → 下降/上昇間の休憩画面 | Fable | ✅ 完了・本番反映済(commits 6b427d3/bc516ed/fbc69c9、317テスト) |
| — | **音域チェック クローズ**: ユーザー実測「楽に出せる範囲=ド3〜シ3」— 較正記録(レ#3〜ド#4で楽に発声)と整合する信頼できる結果。お手本プール=ド3〜シ3の7音に自動適応 | — | ✅ 2026-08-16 |

| T34 | **Phase 6着手: Level 1「音の上下」**(相対音感ドリル。2音再生→「んー、んー」模倣→上下同判定、1セット5問、成長記録に「音の上下」行追加) | Fable(仕様)/Sonnet(実装)/Fable(検証) | ✅ 完了・本番反映済(commit d63de44、**338テスト全PASS**。ユーザー実機確認待ち) |

| T35 | Level 1 全問測定不能事故の修正: 「んーんー」連続歌唱でセグメント分割不能 → 前半/後半フォールバック判定+区切り150ms化+捕捉5秒+文言緩和 | Fable | ✅ 完了・本番反映済(commit d95cfec、342テスト。**ユーザー再挑戦待ち**) |

| T36 | Level 1「2回流れてない」対応: 同一音出題が1回に聞こえる → ♪1つ目/♪2つ目表示+間隔400ms | Fable | ✅ 完了・本番反映済(commit bf99d0d) |
| T37 | **成長記録「音ごとのようす」**(ユーザー要望): noteAbsCents:<midi>で音ごと保存→◎とくい/○まあまあ/△これから一覧 | Fable | ✅ 完了・本番反映済(commit d17b53c、345テスト。データはリリース以降の練習分から蓄積) |

| T38 | **Phase 6 第2弾: Level 3「2音模倣」**(高さ+幅の評価。1問完結型、Level 1様式踏襲、成長記録に「音程の幅」行) | Fable(仕様)/Sonnet(実装)/Fable(検証) | ✅ 完了・本番反映済(commit 42d9923、**370テスト全PASS**) |
| T39 | **+70cent高め傾向対応**(ユーザー報告): お手本音色を純サイン→倍音付きへ(音高知覚の安定化)+方向付きアドバイス閾値±100→±60+「力を抜いて」文言 | Fable | ✅ 完了・同commitで本番反映済 |

| T40 | **「今日のメニュー」セッション化**(基盤→ボイトレ第1段、ユーザー承認): ウォームアップ→データ駆動の重点2つ→しあげ、の10分ガイド。編成=core/menu/buildDailyMenu(ルールベース、将来AI Coach差替可) | Fable(設計・統合)/Sonnet(実装+自主レビューでバグ2件修正)/**Codex(クロスレビュー: 4指摘→全対応**: マイク解放漏れ/音域外出題/重点・しあげ重複/回帰テスト) | ✅ 完了・本番反映済(commit b74cec0、**385テスト全PASS**) |

| T41 | メニュー初回実走フィードバック「4つ全部単音で構造が分からない」→ ステップ案内画面(M-2b)+多様性ルール(単音集中を2連続にしない) | Fable | ✅ 完了・本番反映済(commit e0da3fe、385テスト) |
