...
- [ ] **母音切替・タング系(ユーザー発案 2026-08-16)**: 「アオアオアー」(母音変化=顎・口の開きと
      共鳴の切替。同じ高さを保ったまま母音を変える→歌詞を歌っても音程が揺れない力)と
      「タラタラター」(舌の脱力・滑舌)。測定可能な部分=**母音/舌が動いても F0 を保てるか**
      (既存の pitchStability がそのまま使える。't'の破裂で voicing が瞬断するのは median で吸収見込み、要実測)。
      舌の動き・滑舌の質そのものは測定しない(信頼性高く判定できないものを数値化しない原則)。
      案内文言として Level 2 変種に組み込む。ボイストレーナー監修情報との突き合わせは Phase 6 着手時
