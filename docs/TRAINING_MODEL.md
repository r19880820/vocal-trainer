...
フロー(ExerciseEngineとは独立。録音→オフライン解析 runPipelineOffline を利用):
1. 説明 → 開始タップ → マイク準備+静音500ms(ノイズ測定)
2. **下降パス(RANGE_PASS_SECONDS=6秒)**: 「楽な高さから、少しずつ低く『んー』」
3. **上昇パス(6秒)**: 「楽な高さから、少しずつ高く『んー』」
4. 両パスを解析し結果表示+設定に保存

解析(core/range/analyzeRange.ts、純関数。**2026-08-16 誤測定事故を受け再設計** — 旧設計は
「滞在250ms必要+厳密連続+基準が出せたビンの中央」の複合で、スライド末尾に滞在した端の
小塊(幅3半音)を「楽な範囲」と誤認した):
- voiced サンプルを半音ビンに集計。有声時間 ≥ RANGE_BIN_MIN_MS(150ms)のビン=「出せた音」
- **全力範囲(がんばれば)** = 出せたビンの最低〜最高
- ビンごとの品質: confidence中央値(raw由来)と ジッター(隣接サンプルの|Δcent|中央値)
- **基準は「良い声の側」に固定**: confidence=上位四分位 / ジッター=下位四分位
  (出せたビンの中央から取ると、端に滞在が偏ったとき端が自分自身を基準に合格する循環)
- **「楽に出せる範囲」**= 基準から confidence が RANGE_CONF_DROP 以上落ちず、ジッターが
  RANGE_JITTER_FACTOR 倍以内のビンの最長区間。**未達ビン(速く通過しただけ)は
  RANGE_BIN_GAP_BRIDGE(2半音)まで橋渡しして連続扱い**。品質不合格の達成ビン
  (=実際に声が乱れた証拠)を跨ぐ橋渡しはしない
- 出せたビン < RANGE_MIN_BINS、または**楽な範囲の幅 < RANGE_MIN_COMFORT_BINS(5半音)**
  → 測定失敗(やり直し案内)。狭すぎる結果を自信ありげに返さない(fail-loud)
- UI側の防御: 保存済みの楽な範囲も幅 < RANGE_MIN_COMFORT_BINS なら未測定扱い(修正前の保存データ対策)
