...
解析(core/range/analyzeRange.ts、純関数):
- voiced サンプルを半音ビンに集計。有声時間 ≥ RANGE_BIN_MIN_MS のビン=「出せた音」
- **全力範囲(がんばれば)** = 出せたビンの最低〜最高
- ビンごとの品質: confidence中央値(raw由来)と ジッター(隣接サンプルの|Δcent|中央値)
- 基準 = 中央寄りビンの品質中央値。**「楽に出せる範囲」= 基準から confidence が
  RANGE_CONF_DROP 以上落ちず、ジッターが RANGE_JITTER_FACTOR 倍を超えない連続ビン区間**
- 出せたビン < RANGE_MIN_BINS → 測定失敗(やり直し案内)
