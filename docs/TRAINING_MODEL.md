...
- **ユーザー発声の分割**: voiced区間を無音ギャップ(≥L1_SEGMENT_GAP_MS=250ms)で分割し、
  有声 ≥L1_SEGMENT_MIN_VOICED_MS(300ms)の区間の先頭2つを採用。各区間の midi中央値の差(cent)で方向判定。
  2区間取れなければ測定不能(その問はノーカウントにせず「もう一度」1回だけ再挑戦)
