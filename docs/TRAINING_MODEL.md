...
### Level 4: 短いメロディ — 2026-08-17 v2(Opus設計レビュー Critical5/Major11 を全面反映)

- **曲**: パブリックドメイン童謡を**歌詞付き**で内蔵(M-7裁定: 歌詞で歌う。音節=音の切れ目になり、
  既知曲なので記憶負荷ゼロ、そもそも「歌」になる。「んー」でも可 — 強制しない)。
  曲データ: 音列(C major度数+オクターブ)・音価・休符・歌詞音節・必要音域幅。採用条件: 跳躍≤9半音(m-6)
  - 第1弾: **チューリップ「ドレミ␣ドレミ」(さいたさいた、幅4半音)** と
    **かえるの合唱「ドレミファミレド」(かえるのうたが、幅5半音)**。
    きらきら星(同音連続+幅9半音)は実録音検証後に追加(スコープ裁定)
- **移調はオクターブ単位のみ**。選択 = はみ出し**総cent量**最小(同点は低い側)。範囲は
  comfort → full(がんばれば)→ プリセットの順でフォールバック。幅が足りない曲は選曲画面で非推奨表示(M-5)
- **お手本再生**: AudioContextの**絶対時刻スケジューリング**で1本のタイムラインに全音+休符を配置
  (逐次awaitはジッタで曲に聞こえない — M-6)。音間 L4_NOTE_GAP_MS(100)、同音連続は
  L4_REPEAT_GAP_MS(200)(L1のT36事故の予防 — C-5)。画面は「♪3/7 ソ『み』」+残り時間表示+
  「さいごまで聞いてから」(m-7)。「最初の音だけもう一度」ボタンあり(m-8)
- **評価(v2 — 縮約+オフセット除去+アライメント。ハードスナップ禁止)**:
  1. **ノートイベント抽出**(M-1で確定): ①非voicedが L4_EVENT_BREAK_MS(80ms)以上続いたら
     voicedランを分断 ②ラン内を走査し、現在イベントの**時間重み付き中央値**から±L4_NOTE_STABLE_CENTS(50)
     外れたら新イベント ③各voicedラン先頭50msは捨てる(子音直後の不安定 — M-7)
     ④有声 ≥ L4_NOTE_MIN_MS(150ms)のみ採用 ⑤イベント値=時間重み付き中央値
  2. **縮約正規化(C-1)**: 目標・ユーザー両系列とも「隣接同一音」を1つに畳む
     (ドドソソララソ→ドソラソ。**どちらの歌い方でも同じ結果**。再アタック検出は加点にも減点にも使わない)。
     melodyAccuracy の分母=縮約後の音数
  3. **グローバル・オフセット除去(C-3/C-4)**: ユーザー系列と目標系列の時間重み付き中央値差を推定し除去。
     |オフセット| > L4_KEY_OFFSET_CENTS(150)なら**キーずれ分岐**(誤りではなく「形は合っています!
     出だしの高さだけ合わせてみましょう」+最初の音の再生)
  4. **アライメント(C-2)**: 縮約系列同士を編集距離(Needleman-Wunsch、≤9×9 DP)で対応付け。
     一致判定はスナップではなく**残差 |cents| ≤ L4_NOTE_OK_CENTS(75)**(C-4: PITCH_OK_CENTSの
     「合っている」範囲を別の音名に読む内部矛盾を排除)
  5. melodyAccuracy = 一致数/縮約後音数(skillId は `melodyAccuracy:<songId>` — m-5)
- **捕捉**: 音数×L4_CAPTURE_PER_NOTE_MS(1200)+L4_CAPTURE_TAIL_MS(2500)を上限に、
  発声後の連続無音で早期終了(M-9)。窓終端がvoicedのままなら数系の判定をしない(fail-closed)
- **validity(M-10)**: 有声合計 < L4_VALID_MIN_VOICED_MS(800)または抽出イベント<2 → 測定不能
  (§3.5系の文言・1回だけ再挑戦・SkillSnapshot保存しない)。**「1音ずつはっきり」等の
  歌い方矯正の要求はしない**(M-4 — T35で撤回した判断を再導入しない)
- **フィードバック(1点・責めない)**: 合格=**全目標がmatch**(挿入(ins)のみの相違は合格扱い —
  2026-08-17 実録音検証: 音間の0.16秒のかすり音がinsとして無害に吸収された。指摘しない)
  →「メロディが歌えています!」/ キーずれ分岐(上記)/ 置換kは**歌詞で指摘**
  「『み』のところが{高く/低く}なりました」(m-2)/ 欠落は最初の相違のみ
  「『さい』のあたりをもう一度きいてみましょう」
- **実録音検証記録(M-3、2026-08-17)— 通過**: take1(歌詞、7秒)= チューリップ形一致6/6・
  offset+242→keyOffset・かすり音ins吸収・かえると57%で識別。take2(んー、8.9秒)=
  かえるの合唱形6/7(出だし1半音広め、と正確に診断)・チューリップ50%で識別・
  50cent超で割れた同音はins吸収。歌詞・ハミング両方で抽出成立。
  フレーズ間息継ぎ実測~800ms → 早期終了閾値 L4_SILENCE_END_MS=1500 を確定
- **結果表示(M-8裁定)**: Pitch Curve生軌跡はMVPでは**出さない**(初心者のガタガタ軌跡は
  責めないUX原則に反する)。**音名チップ列の並置**(目標: ド ソ ラ ソ / あなた: ド ソ ラ ソ、
  オフセット除去後)で形を比較。曲線は実地で価値確認後
- **実装順序(M-3 — 机上閾値→実地全滅を3回目にしない)**: ①評価コア(純関数+合成テスト)
  ②**ユーザー実録音での検証**(debugページ手動録音で「チューリップをんー/歌詞で」→
  calibration.analysis相当で抽出パラメータ確認)③UI。②を通過するまで L4_* は仮のまま確定しない
- 同時再生・分割練習(フレーズ境界は曲データに保持=Plan B)・メニュー統合は次段
